...
import csv  # Thêm import csv để đọc file CSV
...
def log_scrape_status(state, city, link, status='STARTED', details='', log_file='scripts/scrape_log.txt'):
    """Append a simple timestamped log entry about the current scraping status to a file."""
    try:
        ...
        with open(log_file, 'a', encoding='utf-8') as f:
            f.write(f"{datetime.now().isoformat()} | {status} | {state} | {city} | {link} | {details}\n")
    except Exception as e:
        logging.error(f"Failed to write scrape log to {log_file}: {str(e)}")
    ...
    with sync_playwright() as p:
        try:
            ...
            try:
                with open(csv_file_path, 'r', encoding='utf-8') as csv_file:
                    ...
                    for row in csv_reader:
                        if len(row) >= 2:  # Ensure row has at least city and state
                            city, state = row[0].strip(), row[1].strip()
                            if city and state:  # Ensure both city and state are non-empty
                ...
                logging.error(f"CSV file not found: {csv_file_path}. Using empty city-state list.")
                city_state_pairs = []
            except Exception as e:
                logging.error(f"Error reading CSV file {csv_file_path}: {str(e)}. Using empty city-state list.")
                city_state_pairs = []
            ...
            for pair in city_state_pairs:
                ...
                # Log start of processing this city/state
                try:
                    log_scrape_status(state, city, link, status='STARTED')
                except Exception:
                    pass
                random_wait(2, 5)
                attempt = 1
                    ...
                    try:
                        ...
                        try:
                            ...
                            if breadcrumb:
                                ...
                                logging.error(f"Breadcrumb div not found for {link}. Skipping to next city-state pair.")
                                break
                        except TimeoutError:
                        ...
                        for item in filter_urls:
                            ...
                            # Log processing of this specific filtered URL
                            try:
                                log_scrape_status(state, city, filter_url, status='FILTER', details=json.dumps(filters, ensure_ascii=False))
                            except Exception:
                                pass
                            attempt_filter = 1
